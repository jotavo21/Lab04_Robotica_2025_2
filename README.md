...
A[Inicio del nodo turtle_controller] --> B[Inicializar ROS2, publisher cmd_vel y cliente reset]
B --> C[Configurar curses en modo no bloqueante]
C --> D[Crear timer cada 0.1 s]


D --> F[Leer tecla con getch]
F --> G{¿Hay tecla?}

G -->|No| H[Publicar velocidad cero]
G -->|Sí| I{¿Tecla de movimiento?}

I -->|Arriba| M1[linear.x = +vel]
I -->|Abajo| M2[linear.x = -vel]
I -->|Izquierda| M3[angular.z = +vel]
I -->|Derecha| M4[angular.z = -vel]

M1 --> Z[Publicar velocidad]
M2 --> Z
M3 --> Z
M4 --> Z

I -->|No| J{¿Tecla es letra?}

J -->|No| H
J -->|Sí| K[Drenar buffer del teclado]

K --> L[Llamar servicio reset]
L --> M[Ejecutar función de dibujo de letra]

M --> N[Mover tortuga con forward, turn, etc.]
N --> O[Detener tortuga cmd_vel = 0]
O --> P[Fin del dibujo]
